...
class Theme extends JObject {
    ...
    public function setBaseUrl($url) {
        $this->_baseUrl = rtrim($this->alias->getAlias($url), '/');
    }
    ...
    public function setBasePath($path) {
        $this->_basePath = $this->alias->getAlias($path);
    }
     ...
     * @throws Exception if [[basePath]] is not set
     */
    public function applyTo($path) {
        ...
        if (empty($pathMap)) {
            ...
            $pathMap = [$this->alias->getAlias('@app') => [$basePath]];
        }

        $path = FileHelper::normalizePath($path);

        foreach ($pathMap as $from => $tos) {
            $from = FileHelper::normalizePath($this->alias->getAlias($from)) . DIRECTORY_SEPARATOR;
            if (strpos($path, $from) === 0) {
                $n = strlen($from);
                foreach ((array) $tos as $to) {
                    $to = FileHelper::normalizePath($this->alias->getAlias($to)) . DIRECTORY_SEPARATOR;
     ...
     * @throws Exception if [[baseUrl]] is not set
     */
